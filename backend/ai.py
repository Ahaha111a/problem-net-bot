...
async def _ask_groq(
    ...
    async def call(model_name):
        ...
        return await groq.chat.completions.create(
            model=model_name, messages=messages, temperature=temp, max_tokens=tokens
        )
    ...
    try:
        ...
        print(f"GROQ PRIMARY ERROR: {type(first_error).__name__}: {first_error}")
        if fallback and fallback != model:
            try:
                ...
                print(f"GROQ FALLBACK ERROR: {type(second_error).__name__}: {second_error}")
        raise first_error
