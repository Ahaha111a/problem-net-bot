...
def _translate_sql(sql: str) -> str:
    ...
    sql = sql.replace("?", "%s")
    insert_ignore = bool(re.search(r"\bINSERT\s+OR\s+IGNORE\s+INTO", sql, flags=re.I))
    sql = re.sub(r"\bINSERT\s+OR\s+IGNORE\s+INTO", "INSERT INTO", sql, flags=re.I)
    # SQLite date helpers -> PostgreSQL equivalents.
    sql = re.sub(r"datetime\('now'\)", "CURRENT_TIMESTAMP", sql, flags=re.I)
    sql = re.sub(r"datetime\(([^,()]+)\)", r"\1::timestamp", sql, flags=re.I)
    sql = re.sub(r"datetime\('now',\s*%s\)", "(CURRENT_TIMESTAMP + (%s)::interval)", sql, flags=re.I)
    sql = re.sub(r"datetime\('now',\s*'([^']+)'\)", r"(CURRENT_TIMESTAMP + INTERVAL '\1')", sql, flags=re.I)
    sql = re.sub(r"date\('now'\)", "CURRENT_DATE", sql, flags=re.I)
    sql = re.sub(r"date\('now',\s*'([^']+)'\)", r"(CURRENT_DATE + INTERVAL '\1')::date", sql, flags=re.I)
    sql = re.sub(r"date\(([^()]+)\)", r"DATE(\1::timestamp)", sql, flags=re.I)
    sql = re.sub(r"strftime\('%H',\s*([^()]+)\)", r"EXTRACT(HOUR FROM \1::timestamp)", sql, flags=re.I)
    sql = re.sub(r"julianday\(([^()]+)\)", r"EXTRACT(EPOCH FROM \1::timestamp) / 86400.0", sql, flags=re.I)
    if insert_ignore and " on conflict " not in sql.lower():
...
def backup_database(backup_dir: str | None = None):
    ...
    if USE_POSTGRES:
        ...
        for table in tables:
            rows = con.execute(f'SELECT * FROM "{table.replace(chr(34), chr(34)*2)}"').fetchall()
...
def _ensure_platform_tables(con):
    ...
    CREATE TABLE IF NOT EXISTS moderator_goals (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        admin_id INTEGER NOT NULL,
        period TEXT NOT NULL,
        target_publish INTEGER DEFAULT 0,
        target_moderate INTEGER DEFAULT 0,
        target_response INTEGER DEFAULT 0,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        UNIQUE(admin_id, period)
    );
