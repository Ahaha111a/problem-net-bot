...
from config import MODERATOR_BOT_TOKEN, CHANNEL_ID, ADMIN_IDS, TIMEZONE, DB_PATH, BACKUP_DIR, DATABASE_URL, REDIS_URL
...
async def main():
    ...
    print(f'🛡 Бот сотрудников запущен. DB={DB_PATH}; backups={BACKUP_DIR}')
    try: await dp.start_polling(bot)
    finally:
        for t in tasks: t.cancel()
        await web_runner.cleanup(); await bot.session.close()
