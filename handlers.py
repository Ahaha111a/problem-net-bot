...
async def switch_to_admin_mode(
    ...
    await message.answer(
        "👨‍💼 <b>Админ-панель</b>\n\n"
        "Вы переключились в режим администратора.\n\n"
        "Здесь доступны модерация, диалоги, "
        "статистика и управление историями.",
        parse_mode="HTML",
...
async def receive_story(
    ...
    # Если историю отправил администратор,
    # возвращаем его в пользовательское меню.
    if is_admin(
        message.from_user.id
    ):
