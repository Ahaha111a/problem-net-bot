...
from database import (
    ...
    get_due_repost_jobs, claim_repost_job, finish_repost_job, get_story,
)
...
async def repost_worker(bot: Bot):
    while True:
        try:
            ...
            jobs = get_due_repost_jobs(now_utc, 10)
            for job in jobs:
                if not claim_repost_job(job['id']):
                    continue
                story = get_story(job['story_id'])
                if not story or not story['post_text']:
                    finish_repost_job(job['id'], 'failed')
                    continue
                try:
                    sent = await bot.send_message(
                        CHANNEL_ID,
                        story['post_text'],
                        reply_markup=channel_story_keyboard(story['id'])
                    )
                    link = await get_channel_message_link(bot, sent.message_id)
                    if link:
                        await sent.edit_reply_markup(reply_markup=channel_story_keyboard(story['id'], link, get_story_reaction_counts(story['id'])))
                    finish_repost_job(job['id'], 'published')
                    log_admin_action(job['admin_id'], 'scheduled_repost', story_id=story['id'], details=f'message_id={sent.message_id}')
                except Exception as e:
                    print(f'REPOST ERROR #{job["id"]}: {e}')
                    finish_repost_job(job['id'], 'failed')
        except Exception as e:
            print(f'REPOST WORKER ERROR: {e}')
...
async def main():
    ...
    repost_task = asyncio.create_task(repost_worker(bot))
